...
const initialTickets = [
  
  {
    ...
    description: 'Customer attempted to pay using Visa ending 1234 but the payment keeps failing.',
  ...
  },
  {
    ...
    description: 'Customer wants to disable auto-renew for their subscription but toggle is disabled.',
    customer: 'Abidul Islam Abid',
    priority: 'MEDIUM',
    status: 'Open',
  ...
  },
  {
    ...
    description: 'Customer placed an order but did not receive an order confirmation email.',
    customer: 'Abidul Islam Abid',
    priority: 'MEDIUM',
    status: 'Open',
  ...
  },
  {
    ...
    description: 'Tax calculated is higher than expected for VAT-registered customers.',
    customer: 'Abidul Islam Abid',
    priority: 'HIGH',
    status: 'Open',
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl p-6 shadow-md">
            <div className="flex items-center justify-between mb-4">
              ...
              {/* <div className="text-sm text-gray-500">Click a card to move it to Task Status</div> */}
            </div>
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <span className={`text-sm px-2 py-1 rounded-full
                         ${statusBadge(ticket.status)}`}>{ticket.status}</span>
                  </div>
                  <p className="text-xs text-gray-600 mt-2 h-14 
                  overflow-hidden">{ticket.description}</p>

                  <div className="mt-4 flex items-center justify-between">
                    <div className="text-xs text-gray-500">#{ticket.id}
                          {ticket.customer}</div>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-1 rounded-full ${priorityBadge(ticket.priority)}`}>{ticket.priority}</span>
                      <div className="text-xs text-gray-400">   <span className="mr-1">📅</span> {ticket.createdAt}</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div>
          <div className="bg-white rounded-xl p-6 shadow-md h-full">
            <div className="flex items-center justify-between mb-4">
              ...
              {/* <div className="text-sm text-gray-400">Select a ticket to add to Task Status</div> */}
            </div>
            ...
            {inProgressTickets.length === 0 ? (
              ...
              <div className="space-y-4">
                {inProgressTickets.map(t => (
                  <div key={t.id} className="p-3 bg-white shadow-2xl rounded-lg
